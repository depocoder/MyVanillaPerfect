...
Обновление обычное: перезапусти игру. Сервер обновлён тогда же.

### Новое на клиенте (звук и погода)

- **Sound Physics Remastered** — эхо в пещерах, приглушение звука за стенами, направление источника. Дальность обработки 256 блоков,
  на шумных фермах количество обрабатываемых звуков за тик ограничено (не просаживает FPS). Настройки — Mod Menu; приезжают один раз, дальше твои.
- **AmbientSounds** (+ CreativeCore) — фоновые звуки леса, ночи, пещер, океана. Громкости — команда /cmdclientconfig в игре.
- **Particle Rain** — объёмный дождь, снег и туман (beta: после дождя тумана нет — известно, ждём следующую версию).

### Новое в мире (только новые чанки)

- **Explorify** — небольшие структуры в ванильном стиле: фермы, святилища, лагеря, мавзолеи, кораблекрушения Энда (реже обычного).
  Таверны Explorify выключены — они дублируют таверны Dungeons and Taverns.
- На сервере: **ChoiceTheorem's Overhauled Village** — переработанные деревни в равнинах, пустыне, горах, саванне, тайге, грибных полях
  (болото/джунгли и аванпосты — за Dungeons and Taverns / Towns and Towers); **Boids** — стаи трески и лосося; **TabTPS** — команды
  /tps, /mspt и /tabtps toggle tab (TPS сервера в Tab) доступны всем.
## 1.2.1 — 2026-09-18
