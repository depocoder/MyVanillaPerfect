...
1. В Prism создай **новый пустой** инстанс: **Minecraft 26.1.2**, загрузчик **Fabric**.
   **НЕ импортируй Fabulously Optimized** и не копируй чужую папку `mods` — packwiz удаляет только те файлы, которые ставил сам, и всё лишнее останется навсегда (старые версии модов рядом с новыми, конфликты, вылет при запуске).
...
### ⚠️ Уже поставил поверх Fabulously Optimized или моды дублируются?

Закрой игру, открой папку инстанса (ПКМ → **Folder**) и **удали папку `mods` целиком**. Запусти игру — packwiz скачает всё заново и дальше будет сам удалять старые версии.
...
**Ресурспаки** пак не раздаёт — ставь свои в `resourcepacks/`, они никуда не денутся.
